...
public class RNBranchModule extends ReactContextBaseJavaModule {
    ...
    public static void initSession(final Uri uri, Activity reactActivity,
            Branch.BranchUniversalReferralInitListener anInitListener) {
    ...
    public static void initSession(Uri uri, Activity reactActivity) {
        ...
        branch.initSession(new Branch.BranchReferralInitListener() {
            ...
            public void onInitFinished(JSONObject referringParams, BranchError error) {
                ...
                try {
                    ...
                    if (referringLink != null)
                        referringUri = Uri.parse(referringLink);

                    result.put(NATIVE_INIT_SESSION_FINISHED_EVENT_PARAMS, referringParams);
                    result.put(NATIVE_INIT_SESSION_FINISHED_EVENT_ERROR,
                            error != null ? error.getMessage() : JSONObject.NULL);
                    result.put(NATIVE_INIT_SESSION_FINISHED_EVENT_URI,
                            referringLink != null ? referringLink : JSONObject.NULL);
            ...
            private void generateLocalBroadcast(JSONObject referringParams, Uri uri,
                    BranchUniversalObject branchUniversalObject, LinkProperties linkProperties, BranchError error) {
                ...
                if (branchUniversalObject != null) {
                    broadcastIntent.putExtra(NATIVE_INIT_SESSION_FINISHED_EVENT_BRANCH_UNIVERSAL_OBJECT,
    ...
    private void forwardInitSessionFinishedEventToReactNative(ReactApplicationContext reactContext) {
        ...
        LocalBroadcastManager.getInstance(reactContext).registerReceiver(mInitSessionEventReceiver,
    ...
    @ReactMethod
    public void showShareSheet(String ident, ReadableMap shareOptionsMap, ReadableMap linkPropertiesMap,
            ReadableMap controlParamsMap, Promise promise) {
        ...
        Runnable myRunnable = new Runnable() {
            ...
            private Runnable init(ReadableMap _shareOptionsMap, String _ident, ReadableMap _linkPropertiesMap,
                    ReadableMap _controlParamsMap, Promise promise, Context context) {
                mPm = promise;
                mContext = context;
            ...
            @Override
            public void run() {
                String messageHeader = shareOptionsMap.hasKey("messageHeader")
                        ? shareOptionsMap.getString("messageHeader")
                        : "";
                ...
                ShareSheetStyle shareSheetStyle = new ShareSheetStyle(mContext, messageHeader, messageBody)
                        .setCopyUrlStyle(mContext.getResources().getDrawable(android.R.drawable.ic_menu_send), "Copy",
                                "Added to clipboard")
                        .setMoreOptionStyle(mContext.getResources().getDrawable(android.R.drawable.ic_menu_search),
                                "Show more")
                ...
                branchUniversalObject.showShareSheet(getCurrentActivity(), linkProperties, shareSheetStyle,
    ...
    @ReactMethod
    public void generateShortUrl(String ident, ReadableMap linkPropertiesMap, ReadableMap controlParamsMap,
    ...
    public void openURL(String url, ReadableMap options) {
        if (mActivity == null) {
            // initSession is called before JS loads. This probably indicates failure to
            // call initSession
            // in an activity.
        ...
        if (options.hasKey("newActivity") && options.getBoolean("newActivity"))
            mActivity.finish();
    ...
    public static BranchEvent createBranchEvent(String eventName, ReadableMap params) {
        ...
        if (params.hasKey("transactionID"))
            event.setTransactionID(params.getString("transactionID"));
        if (params.hasKey("revenue"))
            event.setRevenue(Double.parseDouble(params.getString("revenue")));
        if (params.hasKey("shipping"))
            event.setShipping(Double.parseDouble(params.getString("shipping")));
        if (params.hasKey("tax"))
            event.setTax(Double.parseDouble(params.getString("tax")));
        if (params.hasKey("coupon"))
            event.setCoupon(params.getString("coupon"));
        if (params.hasKey("affiliation"))
        ...
        if (params.hasKey("searchQuery"))
            event.setSearchQuery(params.getString("searchQuery"));
    ...
    public static LinkProperties createLinkProperties(ReadableMap linkPropertiesMap,
            @Nullable ReadableMap controlParams) {
        LinkProperties linkProperties = new LinkProperties();
        if (linkPropertiesMap.hasKey("alias"))
            linkProperties.setAlias(linkPropertiesMap.getString("alias"));
        if (linkPropertiesMap.hasKey("campaign"))
            linkProperties.setCampaign(linkPropertiesMap.getString("campaign"));
        if (linkPropertiesMap.hasKey("channel"))
            linkProperties.setChannel(linkPropertiesMap.getString("channel"));
        if (linkPropertiesMap.hasKey("feature"))
            linkProperties.setFeature(linkPropertiesMap.getString("feature"));
        if (linkPropertiesMap.hasKey("stage"))
            linkProperties.setStage(linkPropertiesMap.getString("stage"));
    ...
    private static Branch setupBranch(Context context) {
        ...
        if (isKeyFromApp) {
            config = new RNBranchConfig(appKey);
        } else {
        ...
        /*
         * This differs a little from iOS. If you add "useTestInstance": true to
         * branch.json but if (mUseDebug || config.getDebugMode()) branch.setDebug();
         * don't add the testKey, on iOS, it will use the test key from the Info.plist
         * if configured. On Android, useTestInstance in branch.json will be ignored
         * unless testKey is present. If mInitialized = true; testKey is not specified
         * in branch.json, it's necessary to add io.branch.sdk.TestMode to } the Android
         * manifest to use the test instance. It's not clear if there's a programmatic
        ...
        if (mUseDebug || config.getDebugMode())
            branch.setDebug();
        return branch;
    }

    // private static Branch setupBranch2(Context context) {
    // Branch branch = Branch.getInstance(context);

    // if (!mInitialized || isKeyFromApp) {
    // Log.i(REACT_CLASS, "Initializing Branch SDK v. " + BuildConfig.VERSION_NAME);
    // RNBranchConfig config;
    // if (isKeyFromApp) {
    // config = new RNBranchConfig(appKey);
    ...
    public ContentMetadata createContentMetadata(ReadableMap map) {
        ...
        if (map.hasKey("price")) {
            ...
            if (map.hasKey("currency"))
                currency = CurrencyType.valueOf(map.getString("currency"));
        ...
        if (map.hasKey("ratingAverage") || map.hasKey("ratingMax") || map.hasKey("ratingCount")) {
            ...
            Integer count = null;
            if (map.hasKey("ratingAverage"))
                average = map.getDouble("ratingAverage");
            if (map.hasKey("ratingCount"))
                count = map.getInt("ratingCount");
            if (map.hasKey("ratingMax"))
                max = map.getDouble("ratingMax");
            metadata.setRating(average, max, count);
        }

        if (map.hasKey("addressStreet") || map.hasKey("addressCity") || map.hasKey("addressRegion")
                || map.hasKey("addressCountry") || map.hasKey("addressPostalCode")) {
            String street = null, city = null, region = null, country = null, postalCode = null;
            if (map.hasKey("addressStreet"))
                street = map.getString("addressStreet");
            if (map.hasKey("addressCity"))
                street = map.getString("addressCity");
            if (map.hasKey("addressRegion"))
                street = map.getString("addressRegion");
            if (map.hasKey("addressCountry"))
                street = map.getString("addressCountry");
            if (map.hasKey("addressPostalCode"))
                street = map.getString("addressPostalCode");
    ...
    public BranchUniversalObject createBranchUniversalObject(ReadableMap branchUniversalObjectMap) {
        ...
        if (branchUniversalObjectMap.hasKey("title"))
            branchUniversalObject.setTitle(branchUniversalObjectMap.getString("title"));
        if (branchUniversalObjectMap.hasKey("canonicalUrl"))
            branchUniversalObject.setCanonicalUrl(branchUniversalObjectMap.getString("canonicalUrl"));
        if (branchUniversalObjectMap.hasKey("contentDescription"))
            branchUniversalObject.setContentDescription(branchUniversalObjectMap.getString("contentDescription"));
        if (branchUniversalObjectMap.hasKey("contentImageUrl"))
            branchUniversalObject.setContentImageUrl(branchUniversalObjectMap.getString("contentImageUrl"));
        ...
        if (branchUniversalObjectMap.hasKey("expirationDate")) {
            ...
            try {
                ...
                Log.w(REACT_CLASS,
                        "Invalid expiration date format. Valid format is YYYY-mm-ddTHH:MM:SS, e.g. 2017-02-01T00:00:00. All times UTC.");
            }
        }
        ...
        if (branchUniversalObjectMap.hasKey("type"))
            branchUniversalObject.setContentType(branchUniversalObjectMap.getString("type"));

        if (branchUniversalObjectMap.hasKey("contentMetadata")) {
            branchUniversalObject
                    .setContentMetadata(createContentMetadata(branchUniversalObjectMap.getMap("contentMetadata")));
        }
    ...
    public void sendRNEvent(String eventName, @Nullable WritableMap params) {
        ...
        Runnable poller = new Runnable() {
            ...
            private Runnable init(ReactApplicationContext _context, Handler _mainHandler, String _eventName,
                    WritableMap _params) {
            ...
            public void run() {
                try {
                    ...
                    if (mContext.hasActiveCatalystInstance()) {
                        ...
                        mContext.getJSModule(DeviceEventManagerModule.RCTDeviceEventEmitter.class).emit(mEventName,
                                mParams);
                    } else {
                        tries++;
                        if (tries <= maxTries) {
    ...
    // Convert an arbitrary ReadableMap to a string-string hash of custom params for
    // userCompletedAction.
